...
describe("Merge Request review", () => {
  ...
  it("checks out the authoritative SHA, runs gates and publishes a merge-ready comment", async () => {
    ...
    const config = autoDevConfigSchema.parse({ repository: { provider: "gitlab", url: `file://${bare}` }, automation: {}, verification: { commands: [{ id: "content", command: "mkdir -p .pnpm-store && touch .pnpm-store/cache && grep -q good feature.txt" }] }, security: {} });
